-- BHARATHI ENTERPRISES - Complete Database Setup 2024 (Fixed)
-- This script handles existing triggers and policies without conflicts
...
DROP POLICY IF EXISTS "Delivery partners can insert their own profile" ON public.delivery_partners;
...
DROP POLICY IF EXISTS "Customers can create their own cart" ON public.cart;
DROP POLICY IF EXISTS "Customers can update their own cart" ON public.cart;
DROP POLICY IF EXISTS "Customers can delete their own cart" ON public.cart;
...
DROP POLICY IF EXISTS "Customers can create their own cart items" ON public.cart_items;
...
INSERT INTO public.products (name, description, price, image_url, category, stock, is_active)
SELECT * FROM (VALUES
...
) AS v(name, description, price, image_url, category, stock, is_active)
...
-- ✅ Handles existing triggers and policies without conflicts
